...
class tournament(commands.Cog):
    ...
    async def new_server(self, server_id : int):
            new__server = {"server_id": server_id, "active": "no", "registeration_channel": 0, "registeration_category": 0, "registration_time": 0, "confimed_role": 0, "slots": 0}
            await tournament.insert_one(new__server)


    async def new_tournament(self, id : int, registeration_channel: int, registeration_category : int, registration_time : 0, confimed_role : int, slots : int):
        if id is not None:
